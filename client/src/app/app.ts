    ...
    <div class="min-h-screen surface-ground">
      ...
      <div class="bg-blue-600 h-4rem flex align-items-center px-4 shadow-2 text-white justify-content-between">
        ...
        <div class="flex align-items-center gap-3">
             ...
             <span class="text-sm opacity-90">Admin Console</span>
             <p-button icon="pi pi-user" [rounded]="true" [text]="true" styleClass="text-white hover:surface-white hover:text-blue-600 transition-colors"></p-button>
        </div>
      </div>
...
export class App implements OnInit {
  ...
  loadLinks() {
    console.log('[App] Loading links from service');
    this.linkService.getLinks().subscribe(data => {
      console.log(`[App] Successfully loaded ${data.length} links`);
      this.links.set(data);
    });
  }
  ...
  openNew() {
      console.log('[App] Opening dialog for new link');
  ...
  generateAlias() {
      ...
      console.log(`[App] Generated random alias: ${result}`);
      this.link.shortCode = result;
  }

  editLink(link: Link) {
      console.log(`[App] Editing link ID: ${link.id}`);
      this.link = { ...link };
      this.linkDialog = true;
  ...
  viewStats(link: Link) {
      console.log(`[App] Viewing stats for link ID: ${link.id}`);
  ...
  loadStats(id: number) {
      this.linkService.getStats(id).subscribe(data => {
          console.log(`[App] Loaded ${data.length} logs for link ID: ${id}`);
          this.logs.set(data);
      });
  }
  ...
  resetStats() {
      ...
      this.confirmationService.confirm({
          ...
          accept: () => {
              console.log(`[App] Resetting stats for link ID: ${this.currentStatsLink!.id}`);
              this.linkService.resetStats(this.currentStatsLink!.id!).subscribe({
                  ...
                  error: (err: any) => {
                      console.error(`[App] Reset stats failed:`, err);
  ...
  exportStats() {
      if (!this.currentStatsLink) return;
      console.log(`[App] Exporting stats for link ID: ${this.currentStatsLink.id}`);
      ...
      this.linkService.exportStats(this.currentStatsLink.id!).subscribe({
          next: (blob) => {
              ...
              console.log(`[App] CSV export triggered`);
          },
          error: (err: any) => {
  ...
  deleteLink(link: Link) {
      this.confirmationService.confirm({
          ...
          icon: 'pi pi-exclamation-triangle',
          accept: () => {
              console.log(`[App] Deleting link ID: ${link.id}`);
  ...
  saveLink() {
      ...
      if (this.link.originalUrl.trim()) {
          console.log(`[App] Saving link (EditMode=${this.isEditMode}):`, this.link);
  ...
  copyToClipboard(code: string) {
    ...
    navigator.clipboard.writeText(fullUrl).then(() => {
      console.log(`[App] Copied to clipboard: ${fullUrl}`);
